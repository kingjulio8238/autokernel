...
- **`target_sol` → `target_speedup` mapping is a heuristic** — `run_suite` currently maps `target_speedup = 1/(1-target_sol)` (sol=0.8 → 5×, sol=0.95 → 20×). This is workload-dependent and not invertible in general. For Phase 2 (SOL UX transition), `MetaOptimizer.run()` should natively accept `target_sol` as a stopping criterion, not a derived speedup.
