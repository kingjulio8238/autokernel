...
| 3a    | ✅ closed 2026-04-23 (code-only; empirical gate deferred) | n/a (synthetic A/B, no compute) | n/a | $0 offline | ~3h across 3 work + 3 review + 2 follow-ups | **Profiler-in-loop for critic.** Threads Modal's `_collect_basic_profile` output end-to-end into the refinement/optimization/reflexion prompts so the LLM sees utilization + bottleneck signal, not just `correct/speedup`. `pipeline-plumbing`: `kernel_agent/prompt_manager.py` three `render_*` methods accept `profile: dict \| None = None`; `kernel_agent/worker.py::_run_remote_eval` captures Modal's `result["profile"]` and `_refine_kernel` threads it in; `modal_infra/app.py::_collect_basic_profile` standardized on 0.0-1.0 fractions for `compute_utilization`/`bandwidth_utilization`/`cache_efficiency`, underscored `bottleneck_type` enum (`compute_bound`/`memory_bound`/`latency_bound`/`unknown`) matching `openkernel/eval/types.py::BottleneckType`, plus `gpu_type`/`hardware_peak_tflops`/`hardware_peak_gbps` for peak labels. Legacy short-form `compute_util`/`bandwidth_util` (0-100) kept for leaderboard/display. `prompt-template`: new "PROFILE FROM LAST ATTEMPT" block in all 3 Jinja templates with `{% if profile %}` gating (no PROFILE section when absent), 4 metric bullets + Triton-aware guidance. `critic-prompt`: `data/prompts/triton_generator_v1.md` + `cuda_generator_v1.md` carry all 4 metrics alongside `{bottleneck_type}`; fill sites wired via new `openkernel/backends/base.py::profile_placeholders` helper (tolerates 0-1 fractions OR 0-100 percentages, defaults missing → 0.0/"unknown") threaded through `openkernel/agents/generator.py::Generator.generate` + `openkernel/engine/inner_loop.py`. Critical correction: `critic_v1.md` has 3 input placeholders (`bandwidth/compute/cache_efficiency`); `bottleneck_type` there is an **output** enum value the critic produces, not a fill-in — only generator templates carry all 4. Review-prompt-quality caught 4 real hallucination/correctness issues (blocking: Haiku would have emitted `float4/uint4` CUDA intrinsics in our Triton-only codebase; medium: `compute_bound` block-size advice misled small-tensor kernels; medium: `latency_bound` "increase grid size" was backwards — grid affects throughput not latency; low: "shared-memory staging" didn't match Triton's implicit caching model). All 4 fixes shipped in task #9 with Triton-aware replacement text + a `float4` regression guard in tests. Review-cost-delta measured raw +438 tokens on the generator template — task #10 trimmed it to net **-37 tokens vs git HEAD** by collapsing 15 lines of instructional scaffolding to 3, and simultaneously fixed a pre-existing bug where 5 never-filled placeholders were leaking literal braces into prompts. Jinja-side guidance came in at ~+350-430 tokens per template after the #9 correctness fixes (above the original +300 internal cap) — accepted the overrun because correctness dominates cost at this scale (~$40/day Opus worst-case, realistic Haiku/Sonnet routing ~$1-8/day) and Phase 3b will expand metrics anyway. Review-pipeline-correctness verdict **clean**: all 4 required keys guaranteed in every Modal code path, graceful partial-profile degradation, no key-name drift that actually breaks downstream. Orchestrator path (NCU-based profiling in `opt_worker_component`, different shape than Modal's basic profile) left with `profile=None` (intentional, flagged as follow-up). Synthetic A/B test (`tests/test_phase3a_wiring.py`, 4/4 passing) proves end-to-end wiring: refinement/reflexion with full profile include all 4 metric lines + Triton-aware guidance substrings + `float4` regression guard; `None`/`{}` produce identical baseline output. Tests: 35 new across 4 files (6 prompt_manager, 18 prompt_templates, 7 critic_prompt, 4 phase3a_wiring), all passing in 0.62s. **Empirical gate DEFERRED**: plan's 30-problem A/B on Phase 1 baseline (`delta_mean_sol ≥ 0.02 AND p95(delta) > 0`) can't fire until Phase 1.D.6 lands a baseline. Phase 3b must not ship until that empirical gate is green. **Modal redeploy REQUIRED before Phase 3a goes live in prod**: v0 still emits old profile shape (0-100 range + hyphenated enum) — graceful degradation but wrong %. Redeploy will happen alongside Phase 1.D.6 sweep. Two non-blockers: orchestrator profile plumbing (NCU-shape adapter needed), `reflexion_prompt.j2` at +429 tokens over the +300 internal guideline. |
| 3b    | not started | —             | —           | —    | —       | —     |
| 3c    | not started | —             | —           | —    | —       | —     |
| 3d    | not started | —             | —           | —    | —       | —     |
| 3e    | not started | —             | —           | —    | —       | —     |
